...
export const crmSchema = z
  .string()
  .regex(/^\d{4,6}\/[A-Z]{2}$/, 'CRM deve estar no formato 12345/UF (ex: 12345/SP)');
...
export const crpSchema = z
  .string()
  .regex(/^\d{2}\/\d{4,6}$/, 'CRP deve estar no formato 00/000000 (ex: 06/123456)');

/**
 * Validador de CRF (Conselho Regional de Farmácia)
 */
export const crfSchema = z
  .string()
  .regex(/^\d{4,6}$/, 'CRF deve conter apenas números (4 a 6 dígitos)');
...
export const crnSchema = z
  .string()
  .regex(/^\d{1}\/\d{4,6}$/, 'CRN deve estar no formato 0/00000 (ex: 3/12345)');
