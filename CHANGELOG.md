...
## [0.17.3] - 2025-10-08
### Changed
- Centraliza o `MessageBus` em mÃ³dulo global (`MessagingModule`) garantindo instÃ¢ncia Ãºnica via `CoreModule`.
- Normaliza paginaÃ§Ã£o em listagens (usuÃ¡rios/pacientes/plan access logs) com clamps padrÃ£o e utilitÃ¡rio `clampLimit`.
- Cria helper `isoStringOrNow` para serializaÃ§Ã£o consistente de datas e adiciona contexto aos logs sensÃ­veis.

### Testing

- Adiciona testes unitÃ¡rios para `LegalTermPresenter` e amplia cobertura do `AnamnesisPresenter`, garantindo serializaÃ§Ãµes opcionais.
...
### Added
- API REST do módulo de agendamento (holds, confirmação, reagendamento e no-show) reutilizando os casos de uso de domínio.
- Migration Supabase `20251009142351_create_scheduling_tables.sql` criando tabelas `scheduling_*`, índices e políticas RLS multi-tenant.

### Changed
- `DomainEvents` agora expõe eventos de scheduling e `SchedulingErrorFactory` cobre falha de resolução de tenant.

### Testing
- `npm run test:unit -- mark-booking-no-show.use-case.spec.ts`
...
- Tabela `anamnesis_metrics` + `AnamnesisMetricsRepository`, permitindo agregados por tenant/dia com contadores de passos, submissÃƒÂµes, tokens, custo e feedback.

- ServiÃƒÂ§o `AnamnesisAIWebhookReplayService` com repositÃƒÂ³rio dedicado (`anamnesis_ai_webhook_requests`) para bloquear replays entre instÃƒÂ¢ncias.

- MigraÃƒÂ§ÃƒÂ£o `1738608000000-UpdateLegalTermsGovernance` adiciona status (`draft/published/retired`) e auditoria (`createdBy/publishedBy/retiredBy`) aos termos legais.
...
- `/legal/terms` expÃƒÂƒÃ‚Âµe status e responsÃƒÂƒÃ‚Â¡veis nas respostas (`LegalTermResponseDto`) e exige o usuÃƒÂƒÃ‚Â¡rio autenticado ao publicar/desativar.



### Security

- Replays do webhook agora sÃƒÂƒÃ‚Â£o bloqueados por chave persistida (`analysisId` + assinatura) ao invÃƒÂƒÃ‚Â©s de cache volÃƒÂƒÃ‚Â¡til.
...
- Endpoints `/legal/terms` permitem criar, publicar e desativar termos legais versionados por tenant, com validaÃƒÂƒÃ‚Â§ÃƒÂƒÃ‚Â£o de acesso multi-tenant.

- Observabilidade reforÃƒÂƒÃ‚Â§ada: logs de acesso ao plano terapÃƒÂƒÃ‚Âªutico, mÃƒÂƒÃ‚Â©tricas de turnaround/tokens/custo e alerta de latÃƒÂƒÃ‚Âªncia configurÃƒÂƒÃ‚Â¡vel.
...
- Webhook `/anamneses/:id/ai-result` agora exige assinatura HMAC (`x-anamnesis-ai-signature` + `x-anamnesis-ai-timestamp`) com janela configurÃƒÂƒÃ‚Â¡vel (`ANAMNESIS_AI_WEBHOOK_MAX_SKEW_MS`).
...
- BaseUseCase e UseCaseWrapper passam a lanÃƒÂƒÃ‚Â§ar exceÃƒÂƒÃ‚Â§ÃƒÂƒÃ‚Âµes diretamente e expÃƒÂƒÃ‚Âµem executeOrThrow, eliminando a dependÃƒÂƒÃ‚Âªncia de unwrapResult nos controllers e garantindo propagaÃƒÂƒÃ‚Â§ÃƒÂƒÃ‚Â£o uniforme de erros.

- Controllers e casos de uso de Auth, Users, Patients e Anamnesis atualizados para o novo contrato; suites unitÃƒÂƒÃ‚Â¡rias, de integraÃƒÂƒÃ‚Â§ÃƒÂƒÃ‚Â£o e E2E ajustadas para refletir o comportamento.

- .gitignore agora ignora a pasta payloads, evitando sujar o repositÃƒÂƒÃ‚Â³rio com artefatos dos fluxos manuais de anamnese.



### Documentation

- README atualizado com as mÃƒÂƒÃ‚Â©tricas da bateria manual de 29/09, novo total de 248 testes automatizados (~18 s) e referÃƒÂƒÃ‚Âªncia de cobertura preservada em 100%.
