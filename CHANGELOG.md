...
### Added
- Tabela `anamnesis_metrics` + `AnamnesisMetricsRepository`, permitindo agregados por tenant/dia com contadores de passos, submissÃµes, tokens, custo e feedback.
- ServiÃ§o `AnamnesisAIWebhookReplayService` com repositÃ³rio dedicado (`anamnesis_ai_webhook_requests`) para bloquear replays entre instÃ¢ncias.
- MigraÃ§Ã£o `1738608000000-UpdateLegalTermsGovernance` adiciona status (`draft/published/retired`) e auditoria (`createdBy/publishedBy/retiredBy`) aos termos legais.
...
- `/legal/terms` expÃÂµe status e responsÃÂ¡veis nas respostas (`LegalTermResponseDto`) e exige o usuÃÂ¡rio autenticado ao publicar/desativar.

### Security
- Replays do webhook agora sÃÂ£o bloqueados por chave persistida (`analysisId` + assinatura) ao invÃÂ©s de cache volÃÂ¡til.
...
- Endpoints `/legal/terms` permitem criar, publicar e desativar termos legais versionados por tenant, com validaÃÂ§ÃÂ£o de acesso multi-tenant.
- Observabilidade reforÃÂ§ada: logs de acesso ao plano terapÃÂªutico, mÃÂ©tricas de turnaround/tokens/custo e alerta de latÃÂªncia configurÃÂ¡vel.
...
### Security
- Webhook `/anamneses/:id/ai-result` agora exige assinatura HMAC (`x-anamnesis-ai-signature` + `x-anamnesis-ai-timestamp`) com janela configurÃÂ¡vel (`ANAMNESIS_AI_WEBHOOK_MAX_SKEW_MS`).
...
### Changed
- BaseUseCase e UseCaseWrapper passam a lanÃÂ§ar exceÃÂ§ÃÂµes diretamente e expÃÂµem executeOrThrow, eliminando a dependÃÂªncia de unwrapResult nos controllers e garantindo propagaÃÂ§ÃÂ£o uniforme de erros.
- Controllers e casos de uso de Auth, Users, Patients e Anamnesis atualizados para o novo contrato; suites unitÃÂ¡rias, de integraÃÂ§ÃÂ£o e E2E ajustadas para refletir o comportamento.
- .gitignore agora ignora a pasta payloads, evitando sujar o repositÃÂ³rio com artefatos dos fluxos manuais de anamnese.

### Documentation
- README atualizado com as mÃÂ©tricas da bateria manual de 29/09, novo total de 248 testes automatizados (~18 s) e referÃÂªncia de cobertura preservada em 100%.
