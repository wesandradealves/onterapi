...
### Autentica��o do Webhook
- Envie os headers `x-anamnesis-ai-timestamp` (timestamp em ms) e `x-anamnesis-ai-signature` (`sha256=<hash>`) em todas as chamadas.
- A assinatura � calculada como `HMAC_SHA256(secret, timestamp + '.' + bodyJson)`.
- Requisi��es com timestamp fora da janela configur�vel (`ANAMNESIS_AI_WEBHOOK_MAX_SKEW_MS`, padr�o 300000 ms) s�o rejeitadas.
